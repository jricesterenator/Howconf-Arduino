/*
  Lab 8. Simon says. Messages over serial will tell your device what to do.
  
  What should happen?
  When the device starts up, it'll be waiting for a pre-programmed sequence
  to be sent over serial. You can do it manually from the serial console, 
  for example:
  TWIST IT
  WAVE IT
  TWIST IT
  ...
  It will then make you do those actions.
  
  Learn:
  - Reading from serial.
  - Using serial to get input data into the Arduino.
...
//If false, the game is in normal single player mode. If true,
//it gets direction from the Serial port.
#define SIMON_SAYS true
...
#define NUM_LEVELS 5
#define MAX_NUM_SIMON_SAYS_LEVELS 100
...
int game_state;
int level;
int simon_says_level = 0;
int simon_says_max_level = -1;
int * levels;
int maxLevel;
...
void setup() {
  ...
  maxLevel = NUM_LEVELS;
  if(SIMON_SAYS) {
    maxLevel = MAX_NUM_SIMON_SAYS_LEVELS; //Allow Simon Says up to 100 levels
  }
  game_init(maxLevel, LED_1, LED_2, LED_3);
  reset_game();
  
}

boolean simon_has_max_level() {
  return simon_says_max_level != -1;
}
  

void loop() {
  
  //If max level is set, not accepting any more inputs
  if(!simon_has_max_level()) { 
    ...
    if(msg == "PUSH IT") {
      levels[simon_says_level++] = PUSH;
      Serial.println("Simon says push!");
      
    } else if(msg == "TWIST IT") {
      levels[simon_says_level++] = TWIST;
      Serial.println("Simon says twist!");
      
    } else if(msg == "WAVE IT") {
      levels[simon_says_level++] = WAVE;
      Serial.println("Simon says wave!");
      
    } else if(msg == "END") {
      simon_says_max_level = simon_says_level;
      Serial.println("Game learned, no more levels.");
    ...
    if(simon_says_level > 100) {
      Serial.println("Too many levels. No more!");
      simon_says_max_level = simon_says_level;
    } 
  }    
...
int getUserInput() {
  ...
  if(abs(potVal - prevPotState) > POT_TRIGGER_LEVEL) {
    return TWIST;
  }
...
void game_loop() {
  ...
  if(game_state == STATE_WIN) {
    ...
    */
    ...
    if(SIMON_SAYS) {
      //Win detection
      if(simon_has_max_level() && level >= simon_says_max_level) {
        game_state = STATE_WIN;
        return;
      }
      
      //If we're caught up to simon, wait.
      if(simon_says_level <= level) {;
        return;
      }

      } else {
      //Win detection
      if(level >= maxLevel) { //single player win
        game_state = STATE_WIN;
        return;
      }
    ...
    for(int l=0; l<level; ++l) {
      prevPotState = analogRead(POT); //Reset pot state so we have a good baseline for next time
